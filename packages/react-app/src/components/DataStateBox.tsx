    ...
    >
      {isEmpty(data)
        ? <div className="w-full h-full flex justify-center items-center opacity-70">
          <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description={connected && networkId ? [3, 5777, 31337].includes(networkId) ? "暂无数据" : "请链接 Ropsten 网络" : emptyDesc} style={{ color: "#fff" }} />
        </div>
        : children}
    </div>
  );
